...
@Component(property = { "javax.portlet.name=" + LoginPortletKeys.FAST_LOGIN,
		...
		"service.ranking:Integer=" + Integer.MAX_VALUE }, service = MVCActionCommand.class)
public class LoginMVCActionCommand extends BaseMVCActionCommand {
	...
	protected void doProcessAction(ActionRequest actionRequest, ActionResponse actionResponse) throws Exception {
		...
		if (!Validator.isBlank(login) && !Validator.isBlank(password)) {
			try {
				...
				long userId = AuthenticatedSessionManagerUtil.getAuthenticatedUserId(httpServletRequest, login,
						password, null);
				...
				if (exception instanceof AuthException) {
					...
					if (throwable instanceof PasswordExpiredException || throwable instanceof UserLockoutException) {
						...
						boolean isUserRegistered = Validator.isNotNull(
								UserLocalServiceUtil.fetchUserByEmailAddress(themeDisplay.getCompanyId(), login));
						if (!isUserRegistered) {
							SessionErrors.add(actionRequest, "userNotExit");
							...
							SessionErrors.add(actionRequest, "authFailed");
							//SessionErrors.add(actionRequest, exception.getClass());
							hideDefaultErrorMessage(actionRequest);
							return;
						...
						|| exception instanceof CookieNotSupportedException || exception instanceof NoSuchUserException
	...
	private ActionRequest _getActionRequest(ActionRequest actionRequest, String state) throws Exception {
		...
		String mfaWebDigest = (String) httpSession.getAttribute("MFA_WEB_DIGEST");

		if (!StringUtil.equals(DigesterUtil.digest(state), mfaWebDigest)) {
			throw new PrincipalException("User sent unverified state");
		}

		Key mfaWebKey = (Key) httpSession.getAttribute("MFA_WEB_KEY");

		Map<String, Object> stateMap = _jsonFactory.looseDeserialize(_encryptor.decrypt(mfaWebKey, state), Map.class);
	...
	private LiferayPortletURL _getLiferayPortletURL(HttpServletRequest httpServletRequest, String redirectURL,
		...
		LiferayPortletURL liferayPortletURL = _portletURLFactory.create(httpServletRequest,
				"com_liferay_multi_factor_authentication_web_portlet_MFAVerifyPortlet", plid,
				PortletRequest.RENDER_PHASE);

		liferayPortletURL.setParameter("saveLastPath", Boolean.FALSE.toString());
		liferayPortletURL.setParameter("mvcRenderCommandName", "/mfa_verify/view");
	...
	private void _postProcessAuthFailure(ActionRequest actionRequest, ActionResponse actionResponse) throws Exception {
		...
		PortletURL portletURL = PortletURLBuilder.create(PortletURLFactoryUtil.create(actionRequest,
				liferayPortletRequest.getPortlet(), layout, PortletRequest.RENDER_PHASE)).setRedirect(() -> {
	...
	private void _redirectToVerify(ActionRequest actionRequest, ActionResponse actionResponse, long userId)
			throws Exception {
		...
		String encryptedStateMapJSON = _encryptor.encrypt(key, _jsonFactory.looseSerializeDeep(
				HashMapBuilder.<String, Object>put("requestParameters", actionRequest.getParameterMap()).build()));

		HttpServletRequest httpServletRequest = _portal
				.getOriginalServletRequest(_portal.getHttpServletRequest(actionRequest));

		LiferayPortletURL liferayPortletURL = _getLiferayPortletURL(httpServletRequest,
				PortletURLBuilder.createActionURL(liferayPortletResponse).setActionName("/login/login")
						.setParameter("state", encryptedStateMapJSON).buildString(),
				PortletURLBuilder.createRenderURL(liferayPortletResponse).setRedirect(() -> {
					String redirect = ParamUtil.getString(actionRequest, "redirect");
		...
		actionRequest.setAttribute(WebKeys.REDIRECT, liferayPortletURL.toString());

		HttpSession httpSession = httpServletRequest.getSession();

		httpSession.setAttribute("MFA_USER_ID", userId);
		httpSession.setAttribute("MFA_WEB_DIGEST", DigesterUtil.digest(encryptedStateMapJSON));
		httpSession.setAttribute("MFA_WEB_KEY", key);
	}

	private static final Accessor<Object, String> _STRING_ACCESSOR = new Accessor<Object, String>() {

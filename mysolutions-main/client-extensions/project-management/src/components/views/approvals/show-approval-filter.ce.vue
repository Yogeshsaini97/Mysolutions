    ...
    <div class="modal-header px-40 pt-40 pb-4" style="display: flex;
      ...
      align-items: flex-end;">
        <h4 class="modal-title mb-0 text-default line-height-8">{{ i18n.global.t('FILTER') }}<br /><span
            class="text-dark font-12 opacity-60">{{ i18n.global.t('PROJECT_APPROVALS.PROJECT')}}<br />{{ i18n.global.t('PROJECT_APPROVALS.APPROVALS')}}</span></h4>
      ...
      <div class="row">
        <div class="col-12 col-md-12 mb-4">
          <p class="text-default font-12 mb-2">Select Created by</p>
          <ul>
      ...
      <div class="my-4">
        <div class="row">
        <div class="col-6"> <button class="btn btn-outline-primary btn-block" type="button"  @click="addFilter()"> {{ i18n.global.t('APPLY') }}</button></div>
        <div class="col-6"> <button class="btn btn-outline-secondary btn-block" type="button" @click="clearSelection()"> {{ i18n.global.t('CLEAR') }}</button></div>
       </div>
    </div>
    </div>
</div>
  ...
  import { inject, ref, } from 'vue';
    export default {
       ...
       console.log("ProjectsDataPass",props.ProjectsDataPass);
            ...
            const uniqueStatusValues = [...new Set(props.ProjectsDataPass.map(item => item.createdBy))];
            const addFilter=()=>{

<!-- <template>
  <div class="sd-body" v-if="stakeholderData!=null  && stakeholderData!='Success' && stakeholderData!='No Data Found' && stakeholderData.key!='server error'">
  <div class="grid-container">
    <div class="grid-item">
      {{ i18n.global.t('STAKEHOLDER.ROLE') }}
        <div class="additional-text">{{ stakeholderData?.roleInProject??i18n.global.t('N/A') }}</div>
    </div>
    <div class="grid-item">
      {{ i18n.global.t('STAKEHOLDER.POSTION') }}
        <div class="additional-text">{{ stakeholderData?.position??i18n.global.t('N/A') }}</div>
    </div>
    <div class="grid-item">
      {{ i18n.global.t('STAKEHOLDER.POWER') }}
        <div class="additional-text">{{ stakeholderData?.power?.split(')')[1]??i18n.global.t('N/A') }}</div>
    </div>
    <div class="grid-item">
      {{ i18n.global.t('STAKEHOLDER.INTEREST') }}
        <div class="additional-text">{{ stakeholderData?.interest?.split(')')[1]??i18n.global.t('N/A') }}</div>
    </div>
    <div class="grid-item">
      {{ i18n.global.t('STAKEHOLDER.ORGANIZATION') }}
        <div class="additional-text">{{ stakeholderPassData?.organization??i18n.global.t('N/A') }}</div>
    </div>
    <div class="grid-item">
      {{ i18n.global.t('STAKEHOLDER.NAME') }}
      <div class="additional-text">{{ stakeholderData?.name??i18n.global.t('N/A') }}</div>
    </div>
    <div class="grid-item">
      {{ i18n.global.t('STAKEHOLDER.PHONE_NUMBER') }}
      <div class="additional-text">{{ stakeholderData?.phoneNumber??i18n.global.t('N/A') }}</div>
  </div>
  <div class="grid-item">
    {{ i18n.global.t('STAKEHOLDER.EMAIL_ADDRESS') }}
      <div class="additional-text">{{ stakeholderData?.email??i18n.global.t('N/A') }}</div>
</div>
</div>
<div style="border-bottom: 2px solid #EDEDED;
margin: 20px 54px;"></div>
<div class="grid-container">
  <div class="grid-item">
    {{ i18n.global.t('STAKEHOLDER.MANAGEMENT_STRATEGIS') }}
      <div class="additional-text">{{ stakeholderData?.managementStrategies??i18n.global.t('N/A') }}</div>
  </div>
   <div class="grid-item">
    {{ i18n.global.t('STAKEHOLDER.STACKHOLDER_CLASSIFICATION') }}
      <div class="additional-text">{{ stakeholderPassData?.stakeholderClassification??i18n.global.t('N/A') }}</div>
  </div>
  <div class="grid-item">
    {{ i18n.global.t('STAKEHOLDER.STACKHOLDER_MANAGEMENT') }}
      <div class="additional-text">{{ stakeholderPassData?.stakeholderManagement??i18n.global.t('N/A') }}</div>
  </div>
  <div class="grid-item">
    {{ i18n.global.t('STAKEHOLDER.UNIT') }}
      <div class="additional-text">{{ stakeholderPassData?.unit??i18n.global.t('N/A') }}</div>
  </div>
</div>
<div style="border-bottom: 2px solid #EDEDED;
margin: 20px 54px;"></div>
<div class="grid-container">
  <div class="grid-item">
    {{ i18n.global.t('STAKEHOLDER.DASHBOARD_ACCESS') }}
      <div class="additional-text">{{ (stakeholderPassData?.needDashboardAcess==1)?"Yes":"No" }}</div>
  </div>
  <div class="grid-item">
    {{ i18n.global.t('STAKEHOLDER.SAYEN_ACCESS') }}
      <div class="additional-text">{{ (stakeholderPassData?.needSayenAccess==1)?"Yes":"No" }}</div>
  </div>
  <div class="grid-item">
    {{ i18n.global.t('STAKEHOLDER.EDITING_ACCESS') }}
      <div class="additional-text">{{ (stakeholderPassData?.canEditDashboard==1)?"Yes":"No" }}</div>
  </div>
  <div class="grid-item">
  </div>
</div> 
...
<div class="grid-container">
  <div class="grid-item">
    {{ i18n.global.t('STAKEHOLDER.CREATED_ON') }}
      <div class="additional-text">{{ stakeholderData?.createdOn??i18n.global.t('N/A') }}</div>
  </div>
  <div class="grid-item">
    {{ i18n.global.t('STAKEHOLDER.CREATED_BY') }}
      <div class="additional-text">{{ stakeholderPassData?.createdBy??i18n.global.t('N/A') }}</div>
  </div>
</div>
</div>
<tbody v-else-if="stakeholderData=='Success' ">
    <tr>
        ...
        <td colspan="7"><div style="
          ...
          align-items: center;
          margin-left: 8rem;
          ">No Record Found !!</div></td>
    </tr>
</tbody>
<tbody v-else-if="stakeholderData=='No Data Found' ">
  <tr>
      <ErrorMessageDialogBox message="No Record Found !!"/>
      <td colspan="7"><div style="
        height: 15rem;
...
<tbody v-else-if="stakeholderData!=null && stakeholderData.key=='server error' ">
  <tr>
      <ErrorMessageDialogBox :message="stakeholderData.message"/>
      <td colspan="7"><div style="display: flex;
          flex-direction: column;
          flex-wrap: wrap;
          align-content: center;
          justify-content: center;
          align-items: center;
          height: 24rem; max-height: 20rem;
          margin-left: 8rem;
          "><div><img style=" 
              height: 6rem;
              width: 71px;
          ;" :src="'/documents/d/mysolutions/server-error'" alt="img" /></div><div style="
        height: 4rem;
        display: flex;
        flex-wrap: wrap;
        align-content: center;
        justify-content: center;
        align-items: center;">{{stakeholderData.message}}</div></div></td>
  </tr>
</tbody>
...
<script>
    ...
    export default {
        ...
        setup(props) {
            ...
            const hostUrl = import.meta.env.VITE_TESTING_LIFERAY_ROOT_API +`ods/epm/stakeholder?stakeholderID=${props.id}%26projectID=${ProjectApiId.value}&p_auth=${Liferay.authToken}`;
      ...
      align-items: flex-end;">
                <h4 class="modal-title mb-0 text-default line-height-8 risk-ml">
                    {{ i18n.global.t('STAKEHOLDER.STAKEHOLDER_DETAILS')}}<br /><span
                        class="text-dark font-12 font-weight-800 opacity-60">ID-{{stakeHolderID? stakeHolderID:"N/A"}}</span>
                </h4>

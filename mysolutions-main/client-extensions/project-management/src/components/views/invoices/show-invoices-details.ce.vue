<template >
  <div class="addScroll paginationDown">
    ...
    <table class="table font-weight-600">
      <thead v-if="showHideHeader && invoiceData != null && invoiceData.length > 0 && invoiceData != 'Success' && invoiceData != 'N/A' && paginatedData.length>0">
        <tr>
      ...
      <tbody v-if="invoiceData != null && invoiceData.length > 0 && invoiceData != 'Success' && invoiceData != 'N/A'">
        ...
        <tr v-for="item in paginatedData" :key="item.id" v-if="paginatedData.length>0">
          ...
          item.accountNumber : i18n.global.t('N/A')  }}</span></td> -->
          <!-- <td>{{ item.accountNumber ?
            item.projectNumber : i18n.global.t('N/A')  }}</td> -->
  ...
  item.status? item.status:i18n.global.t('N/A')  }}</span></td>
          <td>{{ item.trxNumber ?
            item.trxNumber : i18n.global.t('N/A')  }}</td>
          <td>{{ item.amountLineItemsOriginal ?
            formatCurrency(item.amountLineItemsOriginal, i18n.global.locale == "ar_SA" ? 'SAR' : 'en-US') : i18n.global.t('N/A')  }}</td>
          <td>{{ item.collectorName ? item.collectorName : i18n.global.t('N/A')  }}<br /></td>
          <td>{{ item.dueDate ? ChangeDateFormat(item.dueDate) : i18n.global.t('N/A')  }}</td>
          <td>{{ item.dueDate ? ChangeDateFormat(item.lastUpdateDate) : i18n.global.t('N/A')  }}</td>
...
import { ChangeDateFormat, formatCurrency,downloadExcel,applyColorToInvoiceStatus } from '../../../utils/utils';
...
const invoiceWatch = (val) => {
  ...
  if (invoiceValue.status >= 200 && invoiceValue.status < 300  && invoiceValue.message == "ok") {
    if (invoiceValue.payload.message == "Success") {
    ...
    else if (invoiceValue.payload.code == 404) {
            console.log("inside success")
            invoiceData.value = "Success";
            return;
        }
      ...
      invoiceData.value.forEach(invoice => {
          if (invoice?.status=='CL') {
            invoice.status = 'Closed';
        }
        if (invoice?.status=='OP') {
          invoice.status = 'Open';
        }
      });
...
watch(selectPageAndFilterAndDownload, (newValue, oldValue) => {
  ...
  if(newValue?.page=="invoice" && newValue.filterCount==null){
...
const toggleSidebarFilter=()=> {
  ...
  const filterJsonData = (data,selectedStatus, dateSelectedStart,dateSelectedInvoice) => {
  console.log( "selectedStatus", selectedStatus, "dateSelectedStart", dateSelectedStart, "datawww", data);

  return data.filter(item => {
    const projectStartDate = item?.dueDate ? new Date(item.dueDate) : null;
    const projectInvoiceDate = item?.lastUpdateDate ? new Date(item.lastUpdateDate) : null;
    const isStatusMatch = selectedStatus === null || Object.values(selectedStatus).includes(item.status);
    const isStartDateInRange = dateSelectedStart == null ||
      ((projectStartDate!= null && dateSelectedStart != null) &&
      ( projectStartDate >= new Date(dateSelectedStart[0]) &&
        projectStartDate <= new Date(dateSelectedStart[1])));

        const isInvoiceDateInRange = dateSelectedInvoice == null ||
      ((projectInvoiceDate!= null && dateSelectedInvoice != null) &&
      ( projectInvoiceDate >= new Date(dateSelectedInvoice[0]) &&
      projectInvoiceDate <= new Date(dateSelectedInvoice[1])));
    return isStatusMatch && isStartDateInRange && isInvoiceDateInRange;
  });
};

const updatefilterFun=(filter)=>{
  updateDownloadFilter({value:filter,"type":"filterTypeApply"});
  let Projectslistvalue = filterJsonData(invoiceData.value,filter?.selectedStatus.length > 0 ? filter.selectedStatus : null,
    (filter?.dateSelectedStart && filter?.dateSelectedStart.length > 0)
      ? filter.dateSelectedStart
      : null,(filter?.dateSelectedInvoice && filter?.dateSelectedInvoice.length > 0)? filter?.dateSelectedInvoice:null)
  ...
  selectInvoiceData.value=formattedDataValue.value;
  }
}

<template>
  <div>
    ...
    <div class="col-12 col-md-12 mb-4 pt-3" v-if="showHideHeader">
                  <div class="row">
                    <p class="text-dark font-12 mb-2"><b>{{ i18n.global.t('PROJECT_SCHEDULE.FILTER_STATUS') }}</b></p>
                    <!-- <div class="col-2">
                      <div class="filter-status">
                        <div class="layer">
                         <div class="filter-check ground">
                            <input type="checkbox" name="dcheckbox" id="dcheckbox" @click="toggleAllChecked($event)">
                            <label for="dcheckbox"> </label>
                          </div>
                          <div class="text">All</div>
                        </div>
                      </div>
                    </div> -->
                    <div class="col-2" v-for="item in uniqueStatusValues">
                      <div class="filter-status">
                        <div class="layer">
                          <div :class="`filter-check ${applyColorToStatusCheckbox(item)}`">
        <input type="checkbox" :name="applyColorToStatusCheckbox(item)" :id="applyColorToStatusCheckbox(item)" v-model="selectedStatus"
            :value="item"  :disabled="checkAllChecked">
                            <label :for="applyColorToStatusCheckbox(item)"> </label>
                          </div>
      ...
      <table id="scheduleTable" class="table">
        <thead v-if="showHideHeader">
          <tr>
            ...
            <th style="min-width: 200px;">{{ i18n.global.t('PROJECT_SCHEDULE.MILESTONES') }}</th>
        ...
        <tbody v-if="taskData != null && taskData.length > 0 && taskData != 'Success' && taskData != 'N/A'" >
          <tr v-for="(task, rowindex) in paginatedData" :key="rowindex" :title="setTooltip(task)" v-if="paginatedData.length>0">
            <!-- <td  class="mtitle mb-0 font-weight-800 user-select-none">{{ rowindex + 1 }} </td> -->
             <td><span :class="`badge rounded-pill font-10 mr-3 ` + applyColorToStatus(task.status)" ><i class='fa-solid fa-circle font-5 align-middle mr-2'></i>{{ task.status ?
        task.status : 'N/A' }}</span></td>
            ...
            <td v-for="(week, index) in weeks" :key="index">
              <a class="beautifullTooltip-container d-flex" href="#" >
                <div   v-for="(day, dayIndex) in weeksDateRange[index]" :key="dayIndex"
    ...
    <el-pagination class="my-50"
    v-model="currentPage"
    :page-size=pageSizeNum
    :total="dataValueList?.length"
    @current-change="handleCurrentChange"
    v-if="(dataValueList?.length)> pageSizeNum"
  >
  </el-pagination>
  </div>
</template>
...
import { ChangeDateFormat, applyColorToStatus,applyColorToStatusCheckbox } from '../../../utils/utils';
...
const schudleWatch = (val) => {
  ...
  if (taskValue.status >= 200 && taskValue.status < 300 && taskValue.message == "ok" ) {
    if ( taskValue.payload.code == "200") {
    ...
    else if (taskValue.payload.code == 404) {
      ...
      console.log("N/A")
      taskData.value = i18n.global.t('N/A') ;
      return;
      ...
      let uniqueStatusValues1 = [...new Set(taskData.value.map(item => item.status))];
      uniqueStatusValues.value=uniqueStatusValues1;
      lastAndFristDate(taskData.value);
      dataValueList.value=taskValue.payload.tasks;
      console.log("dataValueList.value",dataValueList.value);
      return;
...
const parentTaskIdByOnMouse = (val) => {
  const idToFind = val;
  const result = findParentTaskIndexById(idToFind, taskData.value);
...
const findParentTaskIndexById = (idToFind, taskData) => {
  for (let i = 0; i < taskData.length; i++) {
    if (taskData[i].ID === idToFind) {
      ...
      return { parentTaskID: taskData[i].parentTaskID, index: i, taskName:taskData[i].taskName};
    }
  }
...
const setTooltip = (task) => {
  return `${task.taskName}\n\n Planned:${task.baselineStartDate ? ChangeDateFormat(task.baselineStartDate) : ''}\n Actual:${task.baselineFinishDate ? ChangeDateFormat(task.baselineFinishDate) : ''
  } \n\n Dependency: ${parentTaskIdByOnMouse(task.parentTaskID)}
  `;
}
...
const toggleAllChecked = (event) => {
  ...
  checkAllChecked.value = !checkAllChecked.value;
};

const uniqueStatusValues = ref(null);
...
const dataValueList=ref(null);

const filterJsonData = (data, selectedStatus) => {
  if (!Array.isArray(selectedStatus) || selectedStatus.length === 0) {
    // If selectedStatus is not an array or is empty, return all data
    return data;
  }
...
watch(selectedStatus, (newValue, oldValue) => {
  if (newValue && taskData.value && typeof taskData.value === "object") {
    const filteredTasks = filterJsonData(taskData.value, newValue);
    console.log("filteredTasks", filteredTasks);
    lastAndFristDate(filteredTasks);
    dataValueList.value=filteredTasks;
  }
});
...
const paginatedData = computed(() => {
  ...
  const pageValue=dataValueList.value.slice(startIndex, endIndex);
  return pageValue
  });

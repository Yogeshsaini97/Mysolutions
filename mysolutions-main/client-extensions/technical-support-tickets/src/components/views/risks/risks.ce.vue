...
const RiskUrl = import.meta.env.VITE_TESTING_LIFERAY_ROOT_API + `ods/epm/risk/risk-list/?projectID=${ProjectApiId.value}`;
  ...
  <div class="row">
    <div class="col-md-3 border-right">
      ...
      <div class="mt-4">
        ...
        <p class="greytext">STCS - {{ ProjectOverviewData.id ?
      ...
      <div class=" bg-light shadow-none rounded-0 mnh-140p mt-4">
        <div class="card-body">
          ...
          <p v-if="dataRiskApi?.priority!=null"   class="mb-0 font-8 opacity-80">{{ i18n.global.t('PROJECT_OVERVIEW.YOU_HAVE') }}<span
            class="font-weight-800" :class="'font-10 ' +  applyColorToStatus(dataRiskApi?.priority)">{{ dataRiskApi?.Count != null ? dataRiskApi?.Count : 'N/A' }} {{ (dataRiskApi?.priority) != null ? (dataRiskApi?.priority) : 'N/A' }}</span>{{ i18n.global.t('PROJECT_OVERVIEW.RISKS') }}</p>
        </div>

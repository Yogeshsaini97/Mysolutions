    ...
    <div class="modal-header px-40 pt-40 pb-4" style="display: flex;
      ...
      align-items: flex-end;">
        <h4 class="modal-title mb-0 text-default line-height-8">{{ i18n.global.t('FILTER') }}<br /><span
            class="text-dark font-12 opacity-60">{{ i18n.global.t('PROJECT_RISKS.PROJECT') }}<br />{{
        i18n.global.t('PROJECT_RISKS.RISKS') }}</span></h4>
      ...
      <div class="row">
        <div class="col-12 col-md-12 mb-4">
          <p class="text-default font-12 mb-2">{{ i18n.global.t('SELECT_STATUS') }}</p>
        ...
        <div class="col-12 col-md-12 mb-4">
          <p class="text-default font-12 mb-2">{{ i18n.global.t('SELECT_PRIORITY') }}</p>
          <div class="form-check" v-for="item in uniquePriorityValues">
                <input class="form-check-input" type="checkbox" id="gridCheck" 
  ...
  import { inject, ref, } from 'vue';
    export default {
       ...
       console.log("ProjectsDataPass",props.ProjectsDataPass);
            ...
            const uniquePriorityValues = [...new Set(props.ProjectsDataPass.map(item => item.severity))];
            const addFilter=()=>{
              emit('updatefilter',{selectedStatus:selectedStatus.value,selectedPriority:selectedPriority.value})
            }

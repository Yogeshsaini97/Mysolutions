<template >
  <div class="addScroll paginationDown">
    ...
    <!-- <pagination :totalItems="riskData && riskData.length" :itemsPerPage="itemsPerPage"
      ...
      v-if="(filteredData!=null?filteredData.length:riskData?.length)>pageSizeNum"
    >
    </el-pagination>
    <!-- sidebar starts  -->
    <div>
...
const toggleSidebarFilter=()=> {
  ...
  return data.filter(item => {
    ...
    const isPriorityMatch = selectedPriority === null || Object.values(selectedPriority).includes(item.severity);
    return isStatusMatch && isPriorityMatch;
  });
};

const updatefilterFun=(filter)=>{
  let Projectslistvalue = filterJsonData(riskData.value,filter?.selectedStatus.length > 0 ? filter.selectedStatus : null,filter.selectedPriority.length>0?filter.selectedPriority:null)

...
<template><div v-if="actualValue<=100 && plannedValue<=100">
 <div  v-if="actualValue<100 || plannedValue<100"  class="row prog-bar">
    ...
    <div  class="col-6 p-0">
        <p class="m-0">{{ i18n.global.t('ACTUAL')}}<span :class="{
  ...
  actualValueshow +"%" : 'N/A' }}</span>
    ...
    <div class="col-6 p-0">
        <p style="
      margin-left: 10px;
  " class="m-0">{{ i18n.global.t('PLANNED')}}<span class="ml-2 purpletext ">{{
    plannedValueshow ?
...
<div v-else-if="actualValue==100 && plannedValue==100" class="row prog-bar"  >
  <div  class="col-6 p-0">
    <p class="m-0">{{ i18n.global.t('ACTUAL')}}<span  class=" greentext ml-2">{{ actualValueshow ?
actualValueshow : 'N/A' }}%</span>
    </p>
</div>

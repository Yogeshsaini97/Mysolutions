...
const IssueUrl = import.meta.env.VITE_TESTING_LIFERAY_ROOT_API + `ods/epm/issue/issue-list/?projectID=${ProjectApiId.value}`;
  ...
  <div class="row">
    <div class="col-md-3 border-right">
      ...
      <div class="mt-4">
        ...
        <p class="text-grey">STCS - {{ ProjectOverviewData.id ?
      ...
      <div class=" bg-light shadow-none rounded-0 mnh-140p mt-4">
        <div class="card-body">
          ...
          <p v-if="dataissueApi?.priority!=null"  class="mb-0 font-8 opacity-80">{{ i18n.global.t('PROJECT_OVERVIEW.YOU_HAVE') }}<span
            class="font-weight-800" :class="'font-10 ' +  applyColorToStatus(dataissueApi?.priority)">{{ dataissueApi?.Count != null ? dataissueApi?.Count : 'N/A' }} {{ (dataissueApi?.priority) != null ? (dataissueApi?.priority) : 'N/A' }}</span>{{ i18n.global.t('PROJECT_OVERVIEW.ISSUES') }}</p>
        </div>
      </div>

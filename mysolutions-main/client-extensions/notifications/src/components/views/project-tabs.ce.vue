...
const fetchMultipleApi = async () => {
  try {
    ...
    const [response1, response2, response3, response4] = await Promise.all([
      makeRequest(import.meta.env.VITE_TESTING_LIFERAY_ROOT_API + `ods/epm/project?projectID=${ProjectApiId.value}`, 'GET', null)
      // fetch('api3-url'),
      // fetch('api4-url')
    ]);
...
<template>
  <Tabs>
    ...
    <Tab active="true" id="Overview" :title="i18n.global.t('PROJECT_TABS.NOTIFICATIONS')">

...
props: {
  ...
  setup(props, context) {
    ...
    onMounted(async () => {
      try {
        const response = await fetchData(hostUrl + `&page=1&pageSize=${pageSize.value}&p_auth=${Liferay.authToken}`);
    ...
    async function urlEmbed(CurrentPage) {
      const response = await fetchData(hostUrl + `?page=${CurrentPage}&pageSize=${pageSize.value}&p_auth=${Liferay.authToken}`);
      userList.value = response.items;
    }
...
<template>
  <div>
    <div class="">
      ...
      <div class="table-border p-0">
        <div class="container-fluid my-3 mnh-375p  p-0">
          <div class="row p-0">
 ...
 <!-- --------- table for  risk & Issues  list-  -->
    ...
    <div class="pagination" v-if="lastPageNumber!==1" :class='mt-25'>

...
const dataRiskApi=reactive({"payload":null,"Count":null,  "priority":null});
provide('dataRiskApi', dataRiskApi);

const dataissueApi=reactive({"payload":null,"Count":null,  "priority":null});
provide('dataissueApi', dataissueApi);

const crNumber=inject("crNumber");
///////////////

const CurrentPage = reactive({  fileName: ProjectTabs, key: 'ProjectTabs'});
...
// const projectHostUrl='http://localhost:3000/projects';
// const projectHostUrl = import.meta.env.VITE_TESTING_LIFERAY_ROOT_API + `ods/epm/project/project-list/?crNumber=${crNumber.value}`;
...
const breadcrumbs = ref([
  { label:  i18n.global.t('VIEW_ACTIVITY') ,Link:`${window.location.origin}/group/mysolutions/activity-log` }
]);
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      if (response?.payload?.risks) {
        ...
        tempData=risk_issue_priority_count(response?.payload?.risks);
         dataRiskApi.Count=tempData.Count;
         dataRiskApi.priority=tempData.Name;
        console.log("dataRiskApiinutil",dataRiskApi)

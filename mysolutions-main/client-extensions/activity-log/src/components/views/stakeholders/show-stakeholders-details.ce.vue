    ...
    <table class="table font-weight-600">
      ...
      <tbody
        ...
        <tr v-for="(item, index) in paginatedData" :key="index">
          ...
          <td class="font-weight-800">{{ item.organization ? item.organization : 'N/A' }}<br /></td>
  ...
  if (StackHolderValue.status >= 200 && StackHolderValue.status < 300 &&  StackHolderValue.message == "ok" ) {
    if (StackHolderValue.payload.message == "Success") {
      ...
      StackHolderValue.payload.stakeholders.forEach(function (item) {
        ...
        console.log(item?.email?.split("@")?.includes('stcs.com.sa' )  )
      
         if (item.email.split("@").includes('stcs.com.sa')  ) 
         {
...
{
  let Projectslistvalue = StackHolderData.value.filter((item) => {
  return Object.values(filter).includes(item.position);
  });
